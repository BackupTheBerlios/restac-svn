 ...
 *
 */
public class GenericTextContentWriter {
	
	protected HTTPXOutputStream out;
	
	public GenericTextContentWriter(HTTPXOutputStream out) throws ContentConvertingException {
		this.out = out;
	}

	public void write(String text) throws ContentConvertingException {
		byte[] array;
		try {
			array = GenericTextConverter.contentToByteArray(text, this.out.getContentCharset());
			out.write(array);
		} catch (IOException e) {
	...
	public void writeBuffered(String text) throws ContentConvertingException  {
		byte[] array;
		try {
			array = GenericTextConverter.contentToByteArray(text, this.out.getContentCharset());
